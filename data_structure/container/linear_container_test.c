...
static void test_int_linear_container(LinearContainer *linear_container)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        assert(linear_container_get_by_index(linear_container, i,
                                             (void **)&data) == RET_OK);
        assert(data == 2 * i);
        assert(linear_container_set_by_index(linear_container, i, (void *)i) ==
               RET_OK);
        assert(linear_container_find(linear_container, cmp_int, (void *)i) ==
               i);
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        assert(linear_container_get_by_index(linear_container, 0,
                                             (void **)&data) == RET_OK);
        assert(data == 2 * i);
        assert(linear_container_set_by_index(linear_container, 0, (void *)i) ==
               RET_OK);
    }

    i = n - 1;
    assert(linear_container_foreach(linear_container, check_and_dec_int, &i) ==
           RET_OK);
...
static void single_thread_test(void)
{
    ...
    // linear_container = linear_container_darray_create(NULL, NULL);
    // test_int_linear_container(linear_container);
    test_invalid_params();
